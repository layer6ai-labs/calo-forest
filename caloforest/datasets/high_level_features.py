"""
    TAKEN FROM https://github.com/CaloChallenge/homepage/blob/main/code/HighLevelFeatures.py
...
class HighLevelFeatures:
    ...
    def CalculateEPLFeatures(self, eperlayers):
        """ Computes only layer-wise high-level features given the pre-computed eperlayers """
        self.E_tot = eperlayers.sum(axis=-1)

        for idx, l in enumerate(self.relevantLayers):
            self.E_layers[l] = eperlayers[:, idx]

    def _DrawSingleLayer(self, data, layer_nr, filename, title=None, fig=None, 
                         subplot=(1, 1, 1), vmax=None, colbar='alone'):
        ...
        if filename is not None:
            plt.savefig(filename, facecolor='white', bbox_inches='tight', pad_inches=0.0)

    def _DrawShower(self, data, filename=None, title=None, show=True, vmax=None):
    ...
    def DrawAverageShower(self, data, filename=None, title=None, show=True, vmax=None):
        """ plots average of provided showers """
        ret = self._DrawShower(data.mean(axis=0), filename=filename, title=title, show=show, vmax=vmax)
        return ret

    def DrawSingleShower(self, data, filename=None, title=None, show=True, vmax=None):
        ...
        for num, shower in enumerate(data):
            if filename is not None:
                ...
                local_name = None
            ret.append(self._DrawShower(shower, filename=local_name, title=title, show=show, vmax=vmax))
        return ret
